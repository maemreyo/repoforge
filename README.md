...
rf audit --last 20
rf audit --failed
rf audit --action workspace_commit --slow 2000
rf audit --min-bytes 50000
rf audit stats
rf audit stats --since 2026-07-08 --until 2026-07-14
```
...
`rf audit` reads the existing private `audit.jsonl` for the active configuration and returns the most recent matching consumer calls (MCP or CLI), newest first, with `--last`, `--action`, `--failed`, `--slow` (minimum `duration_ms`), and `--min-bytes` (minimum `result_bytes`) filters. `rf audit stats` reads the existing `operation-metrics.json` and reports one row per action — call count, failure rate, average/max duration, average/max result size in bytes, and top failure error codes — so a slow, frequently failing, or context-window-heavy tool is visible without adding any new instrumentation. `result_bytes` is the compact-JSON size of a successful call's result, recorded next to `duration_ms`; it never contains the result content itself, and an unserializable result is recorded as `null` rather than failing the call. Average result size uses only successful payloads whose size was actually observed, so failed calls and serialization-null successes do not dilute the value. Alongside lifetime totals, `operation-metrics.json` keeps bounded daily buckets (30-day retention); `rf audit stats --since DATE` (optionally `--until DATE`) aggregates only that window instead of all-time totals, so a fix's before/after effect on a tool's failure rate, latency, or result size can be isolated. `rf audit stats` with no `--since` is unchanged.
