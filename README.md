...
`rf audit` reads the existing private `audit.jsonl` for the active configuration and returns the most recent matching consumer calls (MCP or CLI), newest first, with `--last`, `--action`, `--failed`, and `--slow` (minimum `duration_ms`) filters. `rf audit stats` reads the existing `operation-metrics.json` and reports one row per action — call count, failure rate, average/max duration, and top failure error codes — so a slow or frequently failing tool is visible without adding any new instrumentation. Alongside lifetime totals, `operation-metrics.json` keeps bounded daily buckets (30-day retention); `rf audit stats --since DATE` (optionally `--until DATE`) aggregates only that window instead of all-time totals, so a fix's before/after effect on a tool's failure rate or latency can be isolated. `rf audit stats` with no `--since` is unchanged.
