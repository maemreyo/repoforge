...
No prompt logging, source/patch/log persistence, full-path persistence, credentials, chain-of-thought, network payload capture, real action replay, generic tracing backend, OpenTelemetry export, production analytics export, or public MCP/CLI tool is introduced.
