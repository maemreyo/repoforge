...
| 3.6 | Deliberately re-issue the exact same `workspace_exec` call twice (same `expected_head_sha`/`expected_fingerprint`) after the first one already completed. | Does not silently re-execute a mutating command against now-stale state — expect `STALE_STATE` on the second call once the tree has actually moved on, or an idempotent replay if nothing changed. | |
