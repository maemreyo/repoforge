...
`workspace_execute_plan` validates acceptance and every immutable binding before admission, then revalidates the plan before each stage. Required-stage failure, cancellation, unexpected mutation, configuration drift, or stale plan identity stops later execution. Partial stage receipts survive process restart and remain queryable internally without embedding raw command output in operation records. A successful iteration stage can provide evidence but cannot populate `last_verification`; only the accepted plan's final verification-enabled profile can do so.
