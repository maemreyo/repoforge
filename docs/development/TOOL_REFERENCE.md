...
`graph` and `next` results include `read_stats`: `provider_processes` (`gh` subprocess launches against GitHub; a process count, not an HTTPS request count — the transport is not directly instrumented, and higher-level operations such as `gh project item-list` may perform more than one network request), `captured_stdout_bytes`, and `provider_process_duration_ms`, broken down per capability, plus `source` (`live_full` for a fresh batched read, `cache` for a TTL cache hit that performed no provider calls). Cache hits additionally report `cache_hit_reason` and `cache_age_ms`; live reads report `cache_miss_reason` so telemetry never has to guess why a result was not served from cache. Graph reads are batched through GraphQL aliases instead of one `gh api` process per issue, so a fresh 40-node graph uses a handful of provider processes rather than one per node; top-level totals are exact, while per-capability entries are shared attribution (one batched request is counted for every capability it carried) and must not be summed. When status or priority metadata is missing on an issue, the graph applies a safe default (`Backlog`/`P3`) and reports one aggregated `METADATA_DEFAULTED` diagnostic listing the missing fields; a metadata gap never marks the issue provider unavailable. Cross-repository sub-issue or blocker references are never hydrated onto the same number in the local repository: they degrade the affected capability and report `CROSS_REPOSITORY_RELATION_UNSUPPORTED`. An edge node whose repository identity is missing, null, or malformed fails the affected capability closed and reports `EDGE_REPOSITORY_IDENTITY_MISSING` — a relation without a verifiable repository is never assumed local. A same-repository blocker outside the traversed subtree degrades `dependencies` and reports `DEPENDENCY_OUTSIDE_SELECTION_SCOPE` naming the unexpanded blockers, so `next` refuses to select the issue until its dependency context is hydrated. Reaching the 200-node traversal budget sets `truncated`/`sub_issues_truncated` and reports `SUB_ISSUE_TRAVERSAL_BUDGET_EXCEEDED` per affected parent instead of enumerating unseen children into the schema-bounded `unavailable` sets. A Project overlay read failure is isolated to `project_overlay` — it is reported as an unavailable capability, never as traversal truncation. A Project listing that stops before wanted issues reports `PROJECT_ITEM_NOT_REACHED` instead of silently appearing complete. `next` selects per-candidate evidence: comments and project-overlay gaps never erase a selectable candidate (only core issue, sub-issues, and dependencies are required for the default readiness decision), candidates missing required evidence are excluded with `CANDIDATE_EVIDENCE_INSUFFICIENT`, and the operation fails closed only when every recommended candidate is blocked. Required-capability evidence is fail-closed: a missing coverage entry, a truncated capability, or a globally incomplete capability makes every candidate that depends on it insufficient — absence of evidence is never sufficiency; only a candidate explicitly listed in a capability's `unavailable` is a localized failure that spares the others. `next` ranks candidates first and then fills the requested `limit` from the selectable set, so a blocked candidate does not mask a later selectable one. The graph cache is fail-closed: it is served only under a provable authority. An `auth_issued` fingerprint is only emitted by the trust boundary that owns the credential lifecycle (the live credential generation — token or installation lease — plus the reviewed authorization-scope digest), so rotating the token, private key, credential reference, or permission scope is automatically a cache miss; a static profile hash is never treated as a credential generation. Until such a provider is wired, the operator-pinned `server.github_read_cache_authority_digest` remains the explicit `manual_legacy` mode, and the cache stays disabled when no authority is provable (`authority_origin` distinguishes `auth_issued` from `manual_legacy` in cache telemetry).
...
Reclamation fails closed: a worker whose identity cannot be proven while it may still run blocks the replacement (`STALE_EXECUTION_WORKER_IDENTITY_UNPROVEN`), a SIGKILL survivor blocks it (`STALE_EXECUTION_WORKER_RECLAMATION_FAILED`), a truncated registry scan blocks it (`EXECUTION_WORKER_REGISTRY_SCAN_INCOMPLETE`), and an unreadable registry record blocks it (`EXECUTION_WORKER_REGISTRY_UNREADABLE_RECORDS`). The durable ProcessLease registry adds its own fail-closed gates: a REGISTERED/READY spawn-fence lease blocks it (`PROCESS_LEASE_INCOMPLETE`), a lease and binding registry that disagree in either direction — an active lease with no binding, an active binding with no active lease, a pid or process-identity mismatch, or a status split-brain — blocks it (`PROCESS_LEASE_BINDING_DIVERGENCE`), and a reaped worker whose lifecycle could not be persisted durably blocks it (`PROCESS_LEASE_PERSISTENCE_FAILURE`; the registry still shows the worker active, so the pass never reports it reclaimed). A process proven alive after SIGKILL is recorded as `KILLED` and an unprovable one as `UNPROVEN` — both stay active concerns and become `TERMINATED` only on a later pass that proves the process and its group are gone. Every switch, rollback, or upgrade runs a **read-only handoff preflight before any stop or `current` swap**: if the registry cannot support the reclamation, the activation/rollback is refused while the healthy runtime keeps serving (`ACTIVATION_PREFLIGHT_FAILED` / `ROLLBACK_PREFLIGHT_FAILED`) instead of stopping the incumbent first and then discovering the replacement cannot start. The supervisor's own startup reconciliation enters resident `FAIL_CLOSED` on the same uncertainty instead of spawning a contending replacement. Only `running` bindings with a recorded start token are ever auto-reaped; a `running` binding without a start token is invalid for new writes, pre-token records read back as `legacy_unproven`, and tokenless/terminal bindings are never reaped by pattern. During a handoff the restarter keeps worker admission `CLOSING` while it drains the incumbent and launches the replacement, but issues a **single-use replacement permit** (F-012) bound to the closing epoch and the target release: the replacement supervisor carries the permit token in its environment and its first worker spawn claims it atomically, so the replacement can spawn during the handoff while every unrelated spawn (the incumbent, a stale permit from a previous handoff, or a different release) stays refused with `WORKER_ADMISSION_REFUSED`. The permit is rotated per launch attempt — a failed candidate's permit is never reusable by a rollback, which issues its own fresh permit bound to the rollback target — and `open_next()` reopens the epoch only after the live record appears, with a reopen failure surfaced as `WORKER_ADMISSION_REOPEN_FAILED`. The spawned worker itself closes the pre-spawn crash window: when its supervisor is provably dead it claims its own pre-spawn intent lease (fills pid/identity, REGISTERED → READY), writes the binding projection, then acknowledges READY → RUNNING — so a parent that died between `create_intent` and `record_pid` leaves an orphan the reconciler can still prove, reclaim, and archive, and a crash between the claim and the binding write leaves a READY-with-pid lease the reconciler reclaims from the lease alone. The claim is fail-closed on every orphan path: the supervisor is only `alive` when its recorded identity still matches the pid (a reused pid is dead, never a live owner), a dead supervisor with no lease makes the worker self-terminate (it would otherwise be an invisible orphan), and a worker that cannot prove its own identity records the diagnostic pid and self-terminates. Lifecycle persistence is canonical-first and crash-safe: a terminal outcome lands on the canonical ProcessLease BEFORE the binding projection is touched (a lease write failure leaves the binding intact as the recovery projection, never archived first), a reaped worker whose canonical lease is missing or whose store is unwired is reported as a `PROCESS_LEASE_PERSISTENCE_FAILURE` instead of a success, and a terminal archive removes the SQLite shadow row with the JSON record so parity stays in sync after every worker lifetime. The reclamation evidence additionally reports `maintenance_failures`/`maintenance_failure_ids` (terminal-record sweeps that raised — reported, never swallowed) and `terminal_binding_debt` (terminal bindings still in the active store; repairable maintenance, never a live-process divergence).
