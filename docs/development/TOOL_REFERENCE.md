...
| Tool | Purpose |
|---|---|---|
| `workspace_execute_plan` | Execute one exact accepted immutable plan through either its iteration stages or its final full boundary. The call returns a durable `OperationTask` reference immediately; poll with `operation_status`. Every completed stage writes a private, bounded, content-addressed receipt with exact pre/post workspace identity, stage identity, status, artifacts, and mutation evidence. `through="iteration"` never grants commit eligibility. `through="full"` delegates the final stage to the existing authoritative profile runner and becomes commit-eligible only when its exact current verification receipt succeeds. |
| `failure_evidence_read` | Read one exact private `failure_id` produced by plan execution. The response is content-addressed, bounded, secret-redacted, restart-safe, and includes the normalized failure class, stable error code, exact pre/post workspace and environment identities, affected scope, reproducibility, mutation facts, uncertainty, and ordered typed recovery actions. Recovery entries are recommendations with explicit preconditions; they never contain arbitrary command text or grant execution authority. |
...
`workspace_execute_plan` validates acceptance and every immutable binding before admission, then revalidates the plan before each stage. Required-stage failure, cancellation, unexpected mutation, configuration drift, or stale plan identity stops later execution. Partial stage receipts survive process restart and remain queryable internally without embedding raw command output in operation records. Reviewed profiles and diagnostics compile into a deterministic typed DAG. A read-only iteration stage may reuse a private content-addressed cache entry only when workspace/input, stage definition, target argv/profile identity, working directory, environment/toolchain/platform, lockfiles, configuration, repository policy, providers, network policy, dependency receipts, and artifact digests remain compatible. Hits and stable miss reasons are recorded in a new current stage receipt; corrupt or partial artifacts are misses. Mutating stages and the final verification stage are always non-cacheable. A successful iteration stage can provide evidence but cannot populate `last_verification`; only the accepted plan's final verification-enabled profile can do so.
