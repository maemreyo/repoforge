"""MCP tool registration for RepoForge."""
...
SERVER_INSTRUCTIONS = """
RepoForge connects ChatGPT to allowlisted local Git repositories through isolated worktrees.
Always begin with repo_list and repo_context, then create one workspace per task. Inspect before
editing. Prefer exact text replacement or a small validated patch. Review workspace_diff after every
meaningful change. Run workspace_verify before commit; never claim verification succeeded unless the
tool returned success. Commit, push, and create only draft pull requests. Never merge, force-push,
modify protected branches, request secrets, or bypass path/change-budget policies. Use
workspace_restore_paths to safely undo selected uncommitted mistakes after refreshing status.
""".strip()
...
def create_server(config_path: str | Path | None = None) -> FastMCP:
    ...
    @mcp.tool(title="List configured repositories", annotations=READ_ONLY, structured_output=True)
    ...
    @mcp.tool(title="Inspect repository status", annotations=EXTERNAL_READ, structured_output=True)
    ...
    @mcp.tool(title="Read GitHub pull request", annotations=EXTERNAL_READ, structured_output=True)
    ...
    @mcp.tool(title="Read multiple workspace files", annotations=READ_ONLY, structured_output=True)
    ...
    @mcp.tool(title="Write complete file", annotations=LOCAL_DESTRUCTIVE, structured_output=True)
    ...
    @mcp.tool(title="Replace exact text", annotations=LOCAL_DESTRUCTIVE, structured_output=True)
    ...
    @mcp.tool(title="Apply unified patch", annotations=LOCAL_DESTRUCTIVE, structured_output=True)
    ...
    @mcp.tool(title="Commit verified changes", annotations=LOCAL_CREATE, structured_output=True)
    ...
    @mcp.tool(title="Create draft pull request", annotations=EXTERNAL_WRITE, structured_output=True)
    ...
    @mcp.tool(title="Update draft pull request", annotations=EXTERNAL_WRITE, structured_output=True)
    ...
    @mcp.tool(title="Read workspace PR status", annotations=EXTERNAL_READ, structured_output=True)
    ...
    @mcp.tool(title="Read workspace PR checks", annotations=EXTERNAL_READ, structured_output=True)
    ...
    @mcp.tool(title="Remove local workspace", annotations=LOCAL_DESTRUCTIVE, structured_output=True)
