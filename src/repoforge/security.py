"""Path, branch, and patch validation."""
...
def _matches(path: str, pattern: str) -> bool:
    ...
    # Treat a leading **/ as zero or more directories, including repository root.
    if normalized_pattern.startswith("**/"):
        ...
        if line.startswith("diff --git "):
            ...
            for raw in parts[2:]:
                ...
                if raw.startswith(("a/", "b/")):
                    raw = raw[2:]
                paths.append(normalize_relative_path(raw))
            ...
            if raw.startswith(("a/", "b/")):
                raw = raw[2:]
            # The diff --git header is canonical. This catches simple patches without it.
...
def validate_patch(patch: str, repo: RepositoryConfig, *, max_chars: int) -> tuple[str, ...]:
    ...
    # Git patches can encode symlinks (120000) and submodules/gitlinks (160000).
    # Reject all common mode encodings before `git apply`.
    mode_header = re.compile(
        r"^(?:new file mode|deleted file mode|old mode|new mode) (?:120000|160000)$",
        re.MULTILINE,
    )
    index_mode = re.compile(
        r"^index [0-9a-f]+\.\.[0-9a-f]+ (?:120000|160000)$",
        re.MULTILINE,
    )
    if mode_header.search(patch) or index_mode.search(patch):
