...
from ...ports.failure_output_artifact_store import FailureOutputArtifact
...
class FileFailureOutputArtifactStore:
    ...
    def persist(self, content: str) -> FailureOutputArtifact:
        ...
        root = self._state_root / "failure-output-artifacts"
        try:
...
def persist_failure_output(state_root: Path, content: str) -> FailureOutputArtifact:
    return FileFailureOutputArtifactStore(state_root).persist(content)
