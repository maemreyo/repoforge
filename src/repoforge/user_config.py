...
def resolved_config_path(config_path: str | Path) -> Path:
    ...
    return (
        Path(DEFAULT_STATE_ROOT).expanduser().resolve() / "config-locks" / digest / "resolved.toml"
    )
...
def _make_profiles(path: Path) -> tuple[DetectedProfile, ...]:
    ...
    if "fix" in targets:
        profiles.append(
            DetectedProfile(
                "fix", "Run the repository's safe autofix target", False, (("make", "fix"),)
            )
        )
...
def detect_repository_for_setup(
    path: str | Path, repo_id: str | None = None
) -> RepositoryDetection:
...
def lock_generation(lock_path: Path) -> int:
    """Return the current lock generation, accepting version-one locks during migration."""
    if not lock_path.is_file():
        return 0
    raw = read_toml(lock_path)
    lock = raw.get("repoforge_lock")
    if not isinstance(lock, dict):
        raise ConfigError(f"Resolved config has no repoforge_lock table: {lock_path}")
    generation = lock.get("generation", 0)
    if not isinstance(generation, int) or isinstance(generation, bool) or generation < 0:
        raise ConfigError(f"Resolved config generation is invalid: {lock_path}")
    return generation


def build_lock_text(
    config: UserConfig, source_text: str, *, generation: int = 1
) -> tuple[str, list[RepositoryDetection]]:
    """Render an immutable reviewed runtime lock for one accepted generation."""
    if generation < 0:
        raise ConfigError("Resolved config generation cannot be negative")
    detections = [
    ...
    lines = [
        ...
        "[repoforge_lock]",
        f"format_version = {1 if generation == 0 else _LOCK_FORMAT_VERSION}",
    ...
    if generation > 0:
        lines.insert(3, f"generation = {generation}")
...
def resolve_runtime_config_path(path: str | Path) -> Path:
    ...
    generation = lock_generation(lock_path)
